...
public class FunProducto {
    ...
    public String modificarProducto(String preBarCode, String nombre, String marca, String barCode, String precio, String descripcion) throws SQLException {
        ...
        actualizarProductoEnSucursales(preBarCode, producto);
    ...
    private void actualizarProductoEnSucursales(String preBarCode, Producto producto) {
        for (String nombreRegion : sistema.nombresRegiones()) {
            Region region = sistema.obtenerRegion(nombreRegion);
            for (String nombreSucursal : region.nombresSucursales()) {
                Sucursal sucursal = region.obtenerSucursal(nombreSucursal);
                if (sucursal.verificarExistenciaProducto(preBarCode)) {
                    sucursal.modificarProducto(preBarCode, producto);
                }
            }
        }
    }
    ...
    public void eliminarProducto(String barCode) throws SQLException {
        ...
        eliminarProductoEnSucursales(barCode);
        sistema.eliminarProducto(barCode);
        
        JdbcProducto jp = new JdbcProducto();
        jp.delete(producto);
    }
    
    private void eliminarProductoEnSucursales(String barCode) throws SQLException {
        for (String nombreRegion : sistema.nombresRegiones()) {
            Region region = sistema.obtenerRegion(nombreRegion);
            for (String nombreSucursal : region.nombresSucursales()) {
                Sucursal sucursal = region.obtenerSucursal(nombreSucursal);
                if (sucursal.verificarExistenciaProducto(barCode)) {
                    new FunProductoSucursal(sistema, sucursal).eliminarProducto(barCode); //elimina en las sucursales
                }
            }
        }
    }
